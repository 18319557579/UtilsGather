...
class FastListViewHolder<T>(val containerView: View, val holderType: Int) : RecyclerView.ViewHolder(containerView) {
    ...
    fun bind(entry: T, position: Int, func: BindingClosure<T>) {
        //上下文传入的是 FastListViewHolder，用于让 func 中可以调用 gainViewById
        this.apply {
            func(entry, position)
        }
    }
    ...
    //使用缓存
    //todo 但是我觉得这里并不对，因为在 onCreateViewHolder 时，都是不一样的 FastListViewHolder，是不是会存在多个 FastListViewHolder 对象
    fun <T : View?> gainViewById(@IdRes id: Int) : T {
